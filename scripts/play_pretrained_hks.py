...
def load_model(env, pretrained_path, pretrained_config_path, device):
    ...
    with open(
        os.path.join(
            "/home/mikey/skill_transfer/skilful/data/tasks/tasks.json"
        )
    ) as tasks_json:
        tasks = json.load(tasks_json)

        print(flags.env, tasks.keys())
        ...
        if flags.env in tasks.keys():
            ...
            skill_dirs = [(st_home + "/" + skill + ".tar") for skill in skills]
            ...
            flags.foc_options_config_path = [
                st_home + "/skill_config.yaml" for _ in range(len(skill_dirs))
            ]
