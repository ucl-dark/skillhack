...
from agent import is_env_registered, get_env_shortcut
from agent.polybeast import polybeast_env, polybeast_learner
...
def get_learner_flags(flags):
    ...
    lrn_flags["checkpoint"] = os.path.join(
        flags["savedir"], f"{flags.env}.tar"
    )
...
def main(flags: DictConfig):
    ...
    skills_all.PENALTY_PER_STEP = flags["penalty_per_step"]
