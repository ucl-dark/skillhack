...
import nest
import torch
from agent.polybeast.core import file_writer
from agent.polybeast.core import vtrace
from agent.polybeast.models import create_model, losses
from agent.polybeast.models.base import NetHackNet
from agent.polybeast.models.intrinsic import IntrinsicRewardNet
...
def learn(
    ...
    for tensors in learner_queue:
        ...
        if flags.model in ["hks"]:
            ...
            if timestep < flags.hks_max_uniform_time:
                coeff = flags.hks_max_uniform_weight * max(
                    (1 - timestep / flags.hks_max_uniform_time),
                    flags.hks_min_uniform_prop,
                )
            else:
                coeff = (
                    flags.hks_min_uniform_prop * flags.hks_max_uniform_weight
                )

            pot_sm = torch.log(learner_outputs.pot_sm)
            uniform = torch.log(torch.ones_like(pot_sm) / len(pot_sm[0, 0]))

            hks_uniform_loss = coeff * nn.KLDivLoss(
                log_target=True, reduction="batchmean"
            )(pot_sm, uniform)
            print(
                "###",
        ...
        if flags.model in ["hks"]:
            ...
            stats["hks_loss"] = hks_uniform_loss.item()
